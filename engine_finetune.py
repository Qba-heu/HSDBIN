...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,MMD_criterion: torch.nn.Module,
    ...
    for data_iter_step, (samples_all, targets_all) in enumerate(metric_logger.log_every(zip(data_loader,test_loader), print_freq, header)):
        ...
        with torch.cuda.amp.autocast():
            ...
            if epoch>200:
                out_test = model(test_sam,training=True)
                test_logit = torch.nn.functional.softmax(out_test[1], dim=1)

                Align_loss = MMD_criterion(outputs[0],out_test[0],targets,test_logit)
                loss = loss_cls+0.001*Align_loss
            else:
                loss = loss_cls
    ...
    val_acc = val(model,val_loader,'cuda')
    if val_acc>=Best_val:
        Best_val = val_acc
